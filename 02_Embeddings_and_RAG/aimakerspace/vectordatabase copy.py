...
# The implementation of the distance metrics is as follows:

def inner_product_similarity(vector_a: np.array, vector_b: np.array) -> float:
    """Computes normalized inner product similarity.
    Effective for normalized embeddings (which most modern embedding models produce).
    Very fast as it avoids square root operations."""
    return np.dot(vector_a, vector_b)


def euclidean_distance(vector_a: np.array, vector_b: np.array) -> float:
    """Computes the Euclidean distance between two vectors.
    Note: Lower values indicate more similarity."""
...
def jaccard_similarity(vector_a: np.array, vector_b: np.array) -> float:
    ...
    Note: Higher values indicate more similarity."""
